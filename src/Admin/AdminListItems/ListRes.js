...
function ListRes(){
    ...
    function fetchData(){
        ...
        axios
            .get(`${process.env.REACT_APP_URL_SYSTEM_GET_ALL_RESTAURANT}?page=${page}`, {
                headers: {
                    authorization:
                        `Bearer ${token}`
                },
    ...
    function changeStatus(id,status){
        ...
        axios.post(process.env.REACT_APP_URL_SYSTEM_ON_OFF_RES,requestData,{
                headers: {
                    authorization:
                        `Bearer ${token}`,
                },
            ...
            <div className="adminListItemContainer">
                ...
                <table className="adminListItem">
                    <th className="Item_header">
                        ...
                        <td style={{"flexGrow":"2",width:"200px"}}>TÊN</td>
                        <td style={{"flexGrow":"2",width:"100px"}}>SỐ ĐT</td>
                        <td style={{"flexGrow":"13",width:"100px"}}>EMAIL</td>
                    ...
                    {res && res.content.map((item,i)=>{
                        return(
                            <tr key={i} className="Item">
                                ...
                                <td style={{"flexGrow":"6",width:"200px"}}>{item.name}</td>
                                <td style={{"flexGrow":"2",width:"100px"}}>{item.phone}</td>
                                <td style={{"flexGrow":"13",width:"100px"}}>{item.email}</td>
                                <td style={{"flexGrow":"3"}}>
                                    <button onClick={()=>{changeStatus(item._id,item.status===true?"OFF":"ON")}}  style={item.status == true?{"background":"#C7390C"}:{"background":"#245618"}}>{item.status == true? "off":"on"}</button>
